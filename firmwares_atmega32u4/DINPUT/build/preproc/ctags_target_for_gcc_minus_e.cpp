# 1 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino"
...
# 9 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino" 2
# 10 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino" 2
# 55 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino"
...
# 60 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino" 3
                                          __attribute__((__progmem__)) 
# 60 "d:\\work\\flatbox\\firmwares_atmega32u4\\DINPUT\\Flatbox\\Flatbox.ino"
                                                  = {
...
typedef enum {
  CMIDDLE_DOWN, // Middle input is DOWN (Hitbox style)
  CMIDDLE_UP // Middle input is UP (for normal arcade style)
} CardMiddle_t;
CardMiddle_t cmiddle;

void dpad_vinvert(bool, bool, bool, bool);
void dpad(bool, bool, bool, bool);

void (*dpad_p)(bool, bool, bool, bool);

void dpad_vinvert(bool up, bool down, bool left, bool right) {
  dpad(down, up, left, right);
}
...
void dpad(bool up, bool down, bool left, bool right) {
  ...
  if (left && right) {
    if (socd == HITBOX || socd == NEUTRAL) {
      ...
      // SOCD cleaner: Gafro (last input wins in horizontal input)
      if (prevLeft || prevRight) {
        // last one wins. (Not allowed in official tournaments)
        left = !prevLeft;
        right = !prevRight;
      } else {
        // If both came at the exact same time, keep neutral
        left = right = false;
      }
    }
  } else {
...
void setup() {
  ...
  bool updateSOCD = false;
  bool updateCMiddle = false;
  ...
  // Default is middle button = down.
  dpad_p = dpad;

  EEPROM.get(2, cmiddle);

  if (cmiddle != CMIDDLE_DOWN && cmiddle != CMIDDLE_UP) {
    // Initialize to Hitbox style if EEPROM is not initialized
    cmiddle = CMIDDLE_DOWN;
    updateCMiddle = true;
  }

  if (digitalRead(9) == 0x0) {
    // set to standard arcade style setup
    if (cmiddle != CMIDDLE_UP) {
      cmiddle = CMIDDLE_UP;
      updateCMiddle = true;
    }
  }
  ...
  if (updateCMiddle) {
    EEPROM.put(2, cmiddle);
  }

  if (cmiddle == CMIDDLE_UP) {
    // if CMIDDLE_UP is set, re-point the dpad_p pointer to an invert function
    dpad_p = dpad_vinvert;
  }
}
...
void loop() {
  ...
  if (digitalRead(10) == 0x0) {
    report.buttons |= 1 << 1;
    report.crossAxis = 0xff;
  }
  if (digitalRead(14) == 0x0) {
    report.buttons |= 1 << 2;
  ...
  if (digitalRead(20) == 0x0) {
    report.buttons |= 1 << 6;
    report.L2Axis = 0xff;
  }
  if (digitalRead(18) == 0x0) {
    report.buttons |= 1 << 7;
    report.R2Axis = 0xff;
  }
